...
describe("computeExtensionPaths", () => {
  ...
  it("two calls with different agentDirs produce independent results", () => {
    ...
    expect(a.sessionsDir).toBe(join("/agent/a", "sessions"));
    expect(b.sessionsDir).toBe(join("/agent/b", "sessions"));
  });
});
