...
The optional `authorizerChain` field names registered case-by-case decision links (e.g. a light model judge) to consult when a request lands on `ask`, ahead of the interactive prompt. This combined local-fork bundle treats installation as explicit activation and defaults the field to `["safe-allow"]`; set it to `[]` to restore terminal-only behavior.
A downstream extension registers a link via `getPermissionsService().registerAuthorizer(name, authorize, options)`; it decides nothing unless configured, and config order fixes chain order. Registration defaults to `pathEnvelopeMode: "cap-allow"`, which sends reviewer path allows to the terminal; explicit `"honor-reviewer"` preserves them. This local stricter-than-Codex rule still permits reviewed `external_directory` asks and is not an OS-sandbox claim — see [docs/configuration.md](docs/configuration.md#authorizer-chain--case-by-case-decision-links).
