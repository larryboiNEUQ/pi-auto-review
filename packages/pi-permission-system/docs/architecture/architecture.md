...
- `registerAuthorizer(name, authorize, options?)` - register a named live-authority chain link (`allow | deny | defer`, ADR 0007 §4); decides nothing unless named in `authorizerChain`. `options.pathEnvelopeMode` defaults to `cap-allow`, which sends path allows to the terminal; explicit `honor-reviewer` preserves them, while missing authoritative surfaces remain capped. Returns a disposer. This personal Git bundle treats installation as explicit opt-in and supplies `safe-allow` as the configured default; standalone/upstream installs remain operator-activated.
...
The two are one link on a **capability gradient**: the deny/defer reviewer is strictly more restrictive and ships first; the allow-capable adjudicator loosens privilege and is gated behind the full envelope (hard exclusions, structured audit, non-persistence, and explicit operator or bundle activation), because its safety property holds only if the model's decomposition is faithful.

Registration mirrors `registerToolAccessExtractor`: a downstream extension offers a **named** capability (`registerAuthorizer("model-judge", …, options)`) on `permissions:ready`, and this package makes no LLM call itself.
Three invariants govern the seam: config order (not registration order) fixes the security-relevant chain order; a missing configured link is skipped fail-safe (more prompting, never less); and **registration alone grants no authority** — a link decides nothing unless its name is in `authorizerChain`. Upstream activation is operator-authored config; this personal fork treats installation of the combined Git bundle as explicit activation and therefore defaults the chain to `safe-allow`.
Bounded delegation defaults each registration to `pathEnvelopeMode: "cap-allow"`, which downgrades a `path` allow to `defer`; an explicit `honor-reviewer` registration preserves it. Undetermined surfaces remain capped fail-safe. This local fork permits reviewed `external_directory` asks, and deterministic path/external-directory denies resolve before the chain. The reviewer extension owns its provider, prompt, thresholds, Guardian floors, and path-mode choice. The configurable path cap is a local stricter-than-Codex product rule, not a Codex feature or OS sandbox.
...
With principal identity and path portability delivered, this domain has no further genuinely open piece; a non-path serving refinement (additional per-surface `Authorizer` chain exclusions beyond this fork's whole-`path` exclusion) remains a candidate but is not scheduled.
...
│   ├── authorizer-registry.ts `AuthorizerRegistry` (+ `AuthorizerLookup`/`AuthorizerRegistrar` ISP interfaces) - name → link `authorize` + path-envelope mode storage; one instance in `index.ts`, exposed cross-extension via `PermissionsService.registerAuthorizer`; safe mode default, throw-on-duplicate, identity-guarded disposer
│   ├── delegation-envelope.ts `encloseInDelegationEnvelope(authorize, mode)` + `DELEGATION_EXCLUDED_SURFACES` - the bounded-delegation checkpoint (ADR 0007 §5): `cap-allow` caps `path` allows, `honor-reviewer` preserves them, and either mode caps an undetermined surface fail-safe; reviewed `external_directory` asks pass; deny/defer pass through
...
│   ├── authorizer-selection.ts `AuthorizerSelection` class - context-owning `AskEscalator` implementation (`escalate(details)`); selects the terminal once per activation, and per ask resolves `authorizerChain` to registered links and their path-envelope modes (config order; unregistered names skipped fail-safe with an `authorizer_chain_unregistered_link` review event), composes them via `composeAuthorizerChain`, and delegates via `PermissionPrompter`
