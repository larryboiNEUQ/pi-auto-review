...
- `registerAuthorizer(name, authorize)` - register a named live-authority chain link (`allow | deny | defer`, ADR 0007 §4); decides nothing unless its name is present in `authorizerChain` config, and every verdict is capped by the bounded-delegation checkpoint; returns a disposer. This personal Git bundle treats installation as explicit opt-in and supplies `safe-allow` as the configured default; standalone/upstream installs remain operator-activated.
...
Bounded delegation is enforced at a checkpoint that downgrades an excluded-surface `allow` to `defer`. This local fork excludes the `path` surface but permits reviewed `external_directory` asks; deterministic path and external-directory denies still resolve before the authorizer chain. The model's provider/prompt/threshold live in the downstream extension's own config.
...
│   ├── delegation-envelope.ts `encloseInDelegationEnvelope(authorize)` + `DELEGATION_EXCLUDED_SURFACES` - the bounded-delegation checkpoint (ADR 0007 §5): in this local fork, caps a link's `allow` on `path` or an undetermined surface (fail-safe) to `defer`, while permitting reviewed `external_directory` asks; deny/defer pass through
