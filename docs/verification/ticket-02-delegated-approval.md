...
| Cwd, policy, transcript, justification and prior denial | gate facts plus category-budgeted compact session evidence and one-shot override marker; tool results are separately budgeted and default off | `runner.test.ts`, `dossier.test.ts`, `reviewer.test.ts` |
| Secret-safe prompt, forwarding and audit | recursive field/value redaction; forwarded facts validator rejects unredacted credentials; routing audit records only the already selected, redacted evidence projection | `delegated-approval-facts.test.ts`, `forwarding-io.test.ts`, `dossier.test.ts`, `reviewer.test.ts` |
